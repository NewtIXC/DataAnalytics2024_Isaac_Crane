...
#doing some more pca
pca_again<- PCA(nmds_brain, graph=FALSE)
pca_again_again<- PCA(brain_all_use, scale.=TRUE) #this actually gave a nice graph for pca so im keeping it

summary(pca_again)
#visualizing
fviz_pca_ind(pca_again,repel=TRUE, title= "PCA individial samples")
...
set.seed(123)
#predicting optimal clusters
# K-means clustering and WSS for different k values
wss <- numeric(10)  # Store WSS for k = 1 to 10
for (k in 1:10) {
  kmeans_result <- kmeans(brain_num_scaled, centers = k, nstart = 25)  # nstart is the number of random initializations
...
brain_all_pca <- data.frame(pca$x[, 10:118], type = brain_all_scaled$type)
...
brain_all_pca_nn10<- data.frame(pca$x[, 1:10], type = brain_all_scaled$type)
# Split the data into training and testing sets
#use one of the pca's from before 
set.seed(123)
train_index <- createDataPartition(brain_all_pca_nn10$type, p = 0.7, list = FALSE)
train_set <- brain_all_pca_nn10[train_index, ]
test_set <- brain_all_pca_nn10[-train_index, ]
...
# Random Forest -----------------------------------------------------------
#grabbing a pca
brain_all_scaled$type <- gsub("glioblastoma-cell-line", "glioblastoma", brain_all_scaled$type)
brain_all_scaled$type <- gsub("Glioblastoma", "glioblastoma", brain_all_scaled$type)
pcarf<- data.frame(pca$x, type = brain_all_scaled$type)
pcarf10<- data.frame(pca$x[, 1:10], type = brain_all_scaled$type)

pcarf10$type<-as.factor(pcarf10$type)
rf_brainsrc <- rfsrc(type ~ ., data = pcarf10, importance = TRUE)
rf_brain <- randomForest(type ~ ., data = pcarf10, importance = TRUE)
